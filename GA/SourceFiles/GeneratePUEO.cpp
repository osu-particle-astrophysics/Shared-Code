...
vector<vector<float> > Generate(int sections, int genes)
{
  ...
  max_S = 50; //might not need the max variables in this file
  max_H = 50;
  vecrtor<vector<float> > outputVector (sections, vector<float>(genes, 0.0f));
  bool intersect = true;
  for(int i=0; i < sections; i++){
    while(intersect==true){ //while the antenna intersects, generate new values for antena (S, m, H, x_0, y_0, z_0, x_f, y_f, z_f, tau, beta)
      ...
      float beta = distribution_beta(generator);
      tau = 0.26;  //tau must be 0.26
      m = 1;  //we are only evolving m = 1 for now.
