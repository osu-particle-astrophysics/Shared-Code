...
std::vector<std::vector<float> > GeneratePUEO()
{
  ...
  for (int i = 0; i < sections; i++) {
    ...
    { 
      ...
      std::uniform_real_distribution <float> distribution_x_0(0, s);
      x_0 = distribution_x_0(generator);

      std::uniform_real_distribution <float> distribution_y_0(0, x_0);
      y_0 = distribution_y_0(generator);

      std::uniform_real_distribution <float> distribution_z_f(0, h);
      z_f = distribution_z_f(generator);

      std::uniform_real_distribution <float> distribution_y_f(0, z_f);
