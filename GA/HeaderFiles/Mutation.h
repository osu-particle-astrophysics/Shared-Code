...
void Mutation(vector<vector<vector<float> > >& dna_output)
{
  // Start Flag
  ...
  for (int i = reproduction_no; i < crossover_no + reproduction_no; i++)
  {
    for (int j = 0; j < sections; j++)
    {
      for (int k = 0; k < genes; k++)
      {
        ...
        {
          temp.push_back(dna_output[i][j][x]);
        }
        ...
        if (s < mutation_rate / 100.0)
        {
          ...
          while (intersect == true)
          {
            ...
            normal_distribution<float> mutate(dna_output[i][j][k],
                                              (sigma / 100.0)
                                              * dna_output[i][j][k]);
            ...
            {
              intersect = ConstraintPUEO(temp[0], temp[1], temp[2], temp[3],
            ...
            else if (design == "AREA")
            {
              ...
              for (int a = 0; a < sections; a++)
              {
                ...
                {
                  // If the section matches the outer loop,
                  // fill with temp values
                  if (a == j)
                  {
                    antenna[a][b] = temp[b];
                  }
                  // If the section does not match the outer loop, 
                  // will with permanent values
                  else
                  {
                    antenna[a][b] = dna_output[i][a][b];
                  }
                }
              }
