...
class HornAntenna:
    ...
    def check_genes(self):
        ...
        # Load genes
        (side_length, height, x_0, y_0, z_f, y_f, beta, trpzd_length, trpzd_height) = (
            self.genes
        )
        ...
        trpzd_intersect = (trpzd_length * x_0 + trpzd_height * y_0 - x_0 * y_0) / (
            trpzd_height + trpzd_length - y_0
        )

        # Run checks
        if not (0 <= side_length <= self.MAX_S) or not (
            self.MIN_H <= height <= self.MAX_H
        ):
            valid_design = False
        elif not (0 <= x_0 <= x_f):
            valid_design = False
