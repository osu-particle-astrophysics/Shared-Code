...
class GA:
    def __init__(
        self,
        run_name,
        settingsfile="configs/settings.yaml",
        initialization=None,
        gen=None,
    ):
    ...
    def tournament_selection(self):
        ...
        tournament_count = int(self.settings["tournament_rate"] * self.settings["npop"])

        if tournament_count == 0:
            tournament_count = 1

        tournament = self.rng.choice(
            self.population, size=tournament_count, replace=False
        )
    ...
    def roulette_selection(self):
        ...
        probabilities = [
            individual.fitness / total_fitness for individual in self.population
        ]
    ...
    def rank_selection(self):
        ...
        # Calculate the probability of selection for each individual
        probabilities = [
            i / len(self.population) for i in range(1, len(self.population) + 1)
        ]
    ...
    def mutation(self, individual):
        ...
        while not valid_antenna:
            new_gene = self.rng.normal(
                loc=chosen_gene, scale=chosen_gene * self.settings["sigma"]
            )
    ...
    def save_population(self, filepath=None):
        ...
        if filepath is None:
            filepath = (
                self.run_dir / "Generation_Data" / f"{self.generation}_population.pkl"
            )
    ...
    def save_to_tracker(self):
        ...
        with open(filepath, "a", newline="") as file:
            writer = csv.writer(file)
            writer.writerow(
                [self.generation - 1, self.best_fitness, self.best_individual.genes]
            )
    ...
    def get_operator_numbers(self):
        ...
        npop = int(self.settings["npop"])
        mutation_no = int(self.settings["mutation_rate"] * npop)
        crossover_no = int(self.settings["crossover_rate"] * npop)
        reproduction_no = int(self.settings["reproduction_rate"] * npop)

        if crossover_no % 2 != 0:
            crossover_no += 1

        if mutation_no + crossover_no + reproduction_no < npop:
            injection_no = (
                npop - mutation_no - crossover_no - reproduction_no
            )
        else:
            injection_no = 0
            reproduction_no = npop - mutation_no - crossover_no
        ...
        for i in range(int(operator_nos[0] / 2)):
            ...
            while not valid_children:
                ...
                children = self.crossover(
                    parents[parent1_index], parents[parent2_index]
                )

                if self.settings["forced_diversity"]:
                    valid_children = self.test_diverse(
                        children[0], new_population
                    ) and self.test_diverse(children[1], new_population)
                else:
                    valid_children = True
    ...
    def run(self):
        ...
        for i in range(self.settings["n_gen"]):
